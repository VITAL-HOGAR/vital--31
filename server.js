...
app.get('/api/dashboard/stats', async (req, res) => {
    try {
        ...
        res.json({ success: true, data: { patients: patCount || 0, professionals: profCount || 0, educationSessions: eduCount || 0, pendingReports: pendingReports || 0 } });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
});
